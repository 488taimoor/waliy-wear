...
import HealthKit
import Combine
public class WaliyWear: NSObject, WCSessionDelegate {
    
    public let streamSubject = PassthroughSubject<String, Never>()
...
//    private var healthStore = HKHealthStore()
    ...
    func startPublishing(text : String) {
            // Emit values to the stream
        streamSubject.send(text)
    ...
    public   func  getHeartRate(){
          ...
          heartRateModel.autorizeHealthKit(){ [self] value, error in
              if(value != nil){
                  ...
                  startPublishing(text: String(format: "%f", value!))
              }else{
                  sendString(text: "Something went wrong  " , messgaeType: "message")
                  startPublishing(text: "Something went wrong")
              }
                
            }
        startPublishing(text: "76876")
        }
